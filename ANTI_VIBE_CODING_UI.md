...
- Negative letter-spacing (roughly -0.01em to -0.03em) applied to large headings as a default "premium" finishing touch. This is a specific, named utility value (Tailwind's `tracking-tight`/`tracking-tighter`) rather than a neutral typographic choice, and it has become common enough on hero headings to read as a template tell in its own right rather than as deliberate refinement — the same failure as the shadcn button in §16, applied to type instead of components. Tightened tracking is not banned; it must be justified against the type's actual metrics and this product's visual thesis (§6), not applied by default because it is a common convention elsewhere.
...
## Appendix F — Practical Tests: Bike Shop Example and Perigee Landing Page
