...
**The aesthetic-usability effect applies to your own review, not just to end users.** A well-documented HCI finding (Kurosu & Kashimura, 1995, Hitachi Design Center — the original ATM-interface study; corroborated since by Nielsen Norman Group and repeated replications) is that people rate visually attractive interfaces as more usable than they actually are, and that this effect specifically masks *minor* usability problems from detection — it does not mask crashes or fully broken flows, only the kind of friction a quick look-and-judge review is most likely to miss. This means a screenshot that looks clean and polished is weak evidence the underlying workflow is easy to use, including when you are the one judging your own screenshot. Do not let "this looks well-designed" substitute for "I traced the actual task and it worked smoothly" (§17's route-continuity check exists specifically to force the second, harder check).
...
| Dense dashboards with many cards done well (round 2) | A disciplined 4–6/5–7 metric-card range with strong alert contrast is a legitimate, praised professional pattern (Grafana/Datadog-style), not inherently a "vibe coded" tell | Practitioner dashboard-design analysis, independently corroborating this document's own earlier 5–7 figure | Medium | Both this source and the original 5–7 figure trace to practitioner synthesis rather than a single controlled study; treat as convergent practitioner consensus, not proof |
...
- This guide has been simulated against five contrasting product categories (Appendix D), but not validated by an actual independent human design reviewer or real end users. Two independent AI review passes (Appendix E, round 1; Appendix I, round 2) have been completed and their findings integrated — treat these as heuristic review by a second model context, which caught real defects across both rounds (a numbering bug, an overstated WCAG level, several unhedged claims, and whatever Appendix I's round-2 pass found) that same-author self-checks missed, but this is still not equivalent to human user validation or a human design reviewer's judgment.
...
**Round 2** (this audit). Added a Performance and Perceived Quality section (new §14), which required renumbering §14–§24 to §15–§25. Conducted fresh, counter-evidence-focused research (Appendix H's sourcing) and produced a 25-row claim-by-claim audit table (Appendix H). Net substantive changes to the guide body: (1) added the aesthetic-usability effect as evidence that self-review, including an agent's own screenshot judgment, is specifically unreliable for *minor* usability problems — Governing Principle 3; (2) added an explicit blandness/homogenized-restraint counter-risk and two new heuristic tests (Blandness, False-Specificity) — Governing Principle 4, §19; (3) added a route-continuity requirement (walk one complete task across routes, not each route in isolation) — §17, §22, §23, §24; (4) added race-condition/stale-state and state-ownership rows, plus concrete visual-regression-flakiness mitigations — §14; (5) added an exception to component-library de-defaulting for products where staying recognizable is the deliberate, defensible choice — §15; (6) added a compact permission/privacy-communication paragraph — §9; (7) reframed exception-laundering as an instance of Goodhart's Law — §22; (8) replaced the 3-simulation Appendix D with 5, explicitly covering a highly-branded product, a dense/library-recognizable internal tool, cards/dashboards done well, and justified strong motion. Two revisions in Appendix D's simulations were the direct, cited cause of changes 2 and 5 above. A second independent review (Appendix I) was commissioned for this round; its disposition is recorded there.
...
| 25 | §25 dashboard-metric sourcing | "5–7 metrics," "wallpaper within two weeks" | Practitioner secondary sourcing, not raw study data (already hedged in round 1) | None new; round 2 added independent corroboration for the range specifically, which is evidence *for* the range without upgrading the timeframe claim's evidence tier | **Retain, corroborated where corroboration was found, still hedged where it wasn't** |
...
## Appendix I — Round 2 Independent Review Report

*(Pending: a second independent review pass, commissioned from a fresh agent context with no access to this document's revision rationale, is in progress for the round-2 changes above. This appendix will record its findings and disposition once complete — recorded here honestly rather than presented as finished before it has actually run.)*
